...
def login_page():
    ...
    <div class="login-card">
        ...
        <div class="login-title">Placement Intelligence Apex</div>
        <div class="login-sub">AI Powered Placement Analytics</div>
    </div>
    """, unsafe_allow_html=True)
...
def load_and_prepare_data():
    ...
    # ================= AUTO GENERATE DATA =================
    for i in range(1, 9):
        if f"SGPA_Sem" not in df.columns:
            df[f"SGPA_Sem"] = np.random.uniform(6.5, 9.5, len(df)).round(2)
        if f"Backlogs_Sem" not in df.columns:
            df[f"Backlogs_Sem"] = np.random.randint(0, 2, len(df))
        if f"Attendance_Sem" not in df.columns:
            df[f"Attendance_Sem"] = np.random.uniform(70, 95, len(df)).round(1)
...
def generate_narrative_report(df):
    ...
    report["Hiring Performance"] = (
        f" recruited the highest number of students "
        f"with  successful hires, indicating strong "
    ...
    report["Package Analysis"] = (
        f" offers the highest average package "
        f"at    LPA. "
    ...
    report["Branch Performance"] = (
        f"The  branch shows the strongest placement "
    ...
    report["Placement Rate"] = (
        f"The university placement rate is %, "
        f"with  students placed out of ."
    )
    # ---------------- Company Difficulty ----------------
    ...
    report["Company Difficulty"] = (
        f" appears to be the most competitive company "
    ...
    report["Skill Demand"] = (
        f"The most demanded skill among placed students is ."
    )
    # ---------------- Placement Trend ----------------
    yearly = placed.groupby("Year")["StudentID"].nunique()
    best_year = yearly.idxmax()
    report["Placement Trend"] = (
        f"The strongest placement performance occurred in ."
    )
    return report
# ==========================================================
...
def detect_year(question):
    year_match = re.search(r"\b20\d\b", question)
    if year_match:
...
def placement_score(profile):
    sgpa = np.mean([profile[f"SGPA_Sem"] for i in range(1, 9)])
    backlogs = sum([profile[f"Backlogs_Sem"] for i in range(1, 9)])
    skills = len(str(profile.get("Skills", "")).split(","))
    attendance = np.mean([profile[f"Attendance_Sem"] for i in range(1, 9)])
    score = (
        sgpa * 10 * 0.4 +
        (skills * 5) * 0.2 +
        attendance * 0.2 -
...
def train_model_cached(df):
    ...
    model_df["Avg_SGPA"] = model_df[[f"SGPA_Sem" for i in range(1,9)]].mean(axis=1)
    model_df["Total_Backlogs"] = model_df[[f"Backlogs_Sem" for i in range(1,9)]].sum(axis=1)
    model_df["Avg_Attendance"] = model_df[[f"Attendance_Sem" for i in range(1,9)]].mean(axis=1)
...
def rank_students(df, model):
    temp = df.copy()
    temp["Avg_SGPA"] = temp[[f"SGPA_Sem" for i in range(1, 9)]].mean(axis=1)
    temp["Total_Backlogs"] = temp[[f"Backlogs_Sem" for i in range(1, 9)]].sum(axis=1)
    temp["Avg_Attendance"] = temp[[f"Attendance_Sem" for i in range(1, 9)]].mean(axis=1)
...
def interpret_graph(title, data):
    if title == "SGPA":
        ...
        return f"The student's academic performance is  with an average SGPA of ."
    elif title == "Backlogs":
        total = data["Backlogs"].sum()
        return f"The student has  total backlogs across semesters."
    elif title == "Attendance":
        avg = data["Attendance"].mean()
        status = "good" if avg > 75 else "poor"
        return f"Average attendance is %, indicating  consistency."
    elif title == "Subjects":
        top = data.sort_values("Marks", ascending=False).iloc[0]["Subject"]
        return f"The strongest subject appears to be ."
    elif title == "Placement":
        offers = len(data[data["Status"] == "Placed"])
        attempts = len(data)
        rate = round((offers / attempts) * 100, 2) if attempts > 0 else 0
        return f"The student has a placement success rate of %."
...
Ask ONE  level interview question for:
Make it:
- Industry relevant
...
if selected_tab == "Home":
    ...
    with col1:
        st.markdown("""
        <div class="card">
        <h2>AI Placement Intelligence System</h2>
        <p>Advanced analytics + AI predictions + real-time insights for students, recruiters, and placement officers.</p>
        </div>
        """, unsafe_allow_html=True)
        st.write(f"Welcome ****")
    with col2:
    ...
    def kpi(title, value, delta):
        ...
        """, unsafe_allow_html=True)
    with c1:
        kpi("Total Students", total_students, f"/ ", t1) 
    with c2:
        kpi("Companies", companies, t2)
    with c3:
        kpi("Avg Package (LPA)", avg_package, t3)
    with c4:
        kpi("Placement Rate", f"%", t4)
    st.markdown("----")
...
if selected_tab == "Student Dashboard":
    ...
    if not selected_student:
        ...
        if stu_data.empty:
            ...
            # =========================
            sgpas = np.array([profile[f"SGPA_Sem"] for i in range(1,9)])
            backlogs_arr = np.array([profile[f"Backlogs_Sem"] for i in range(1,9)])
            attendance = np.array([profile[f"Attendance_Sem"] for i in range(1,9)])
            avg_sgpa = sgpas.mean()
            total_backlogs = backlogs_arr.sum()
            avg_att = attendance.mean()
            df["Avg_SGPA"] = df[[f"SGPA_Sem" for i in range(1,9)]].mean(axis=1)
            ...
            with tabs[0]:
                ...
                with center:
                    ...
                    for i, step in enumerate(steps):
                        ...
                        flow_html += f'<span style="padding:8px 12px;background:;margin:4px;border-radius:8px;color:white;"></span>'
                    ...
                    cols = [c for c in subject_cols if f"_Sem" in c]
                    sub = [c.replace(f"_Sem","") for c in cols]
                    ...
                    # SKILLS + ACTIVITIES
                    st.markdown('<div class="glass">', unsafe_allow_html=True)
                    chip_html = "".join([f'<span class="chip"></span>' for s in skills])
                    st.markdown("### Skills")
            ...
            with tabs[1]:
                ...
                cols = [c for c in df.columns if f"_Sem" in c and not any(x in c for x in ["SGPA","Attendance","Backlogs"])]
                sub = [c.replace(f"_Sem","") for c in cols]
                marks = [profile[c] for c in cols]
                sdf = pd.DataFrame()
                col1,col2 = st.columns(2)
            ...
            with tabs[4]:
                ...
                st.metric("Selection Probability", f"%")
                st.progress(adj/100)
                st.info(ai_summary(profile))
# =======================
# ADMIN COMPANY ANALYSIS
...
if selected_tab == "Company Dashboard":
    ...
    with company_tabs[4]:
        ...
        if not placed_df.empty:
            ...
            st.success(f"Top Hiring Branch: ")
            st.success(f"Average Package:  LPA")
        difficulty = round(len(comp_df) / max(len(placed_df), 1), 2)
        st.warning(f"Hiring Difficulty Score: ")
        st.markdown("###  Ask AI")
    ...
    if len(audio) > 0:
        with st.spinner("Processing your speech..."):
            ...
            try:
                ...
                st.error(f"Audio processing error: ")
                st.stop()
...
""")
        ...
        c1,c2,c3 = st.columns(3)
        c1.metric("Confidence", f"%")
        c2.metric("Fluency", f"%")
        c3.metric("Clarity", f"%")
        c4,c5,c6 = st.columns(3)
        c4.metric("Filler Control", f"%")
        c5.metric("Professional Language", f"%")
        c6.metric("Structure", f"%")
        c7,c8,c9 = st.columns(3)
        c7.metric("Thinking Flow", f"%")
        c8.metric("Voice Confidence", f"%")
        c9.metric("Speech Speed", f" WPM")
        st.metric("Overall Score", f"%")
